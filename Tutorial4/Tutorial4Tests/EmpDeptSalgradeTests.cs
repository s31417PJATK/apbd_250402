...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var temp = depts.Where(d=>d.Loc == "Chicago").Select(d=>d.DeptNo).ToList();
        List<Emp> result = emps.Where(e => temp.Contains(e.DeptNo)).ToList();
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Where(e => e.Job == "SALESMAN").Select(e => new { e.EName, e.Sal }).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e => grades.Where(s => e.Sal <= s.Hisal && e.Sal >= s.Losal),
            (e, s) => new { EName = e.EName, Grade = s.Grade });
